...
const TypeCompteForm = (props) => {
    ...
    if (!edittypeid) {
        ...
        if (!verifyPermission(props.userPermissions, 'update-account-type'))
            window.location.href = ERROR_401;
    }
    const defaultData = {
        name: "",
    ...
    const onSubmit = (e) => {
        ...
        if (verifyTokenExpire()) {
            if (edittypeid) {
                axios.put(appConfig.apiDomaine + `/account-types/${edittypeid}`, data)
                    .then(response => {
                ...
                axios.post(appConfig.apiDomaine + `/account-types`, data)
                    .then(response => {
                        setStartRequest(false);
                        setError(defaultError);
                        setData(defaultData);
...
export default connect(mapStateToProps)(TypeCompteForm);
