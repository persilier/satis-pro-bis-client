...
const CategoryClient = (props) => {
  ...
  document.title =
    "Satis client - " + ready ? t("Paramètre categorie client") : "";
  ...
  useEffect(() => {
    async function fetchData() {
      axios
        .get(appConfig.apiDomaine + "/category-clients")
        .then((response) => {
          setLoad(false);
  ...
  const filterShowListBySearchValue = (value) => {
    ...
    newCategoryClients = newCategoryClients.filter(
      (el) =>
  ...
  const searchElement = async (e) => {
    if (e.target.value) {
      ...
      setNumberPage(
        forceRound(categoryClient.length / NUMBER_ELEMENT_PER_PAGE)
      );
    ...
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (result.value) {
        if (verifyTokenExpire()) {
          axios
            .delete(
              appConfig.apiDomaine + `/category-clients/${categoryClientId}`
            )
            .then((response) => {
              const newCategory = [...categoryClient].filter(
                (e) => e.id !== categoryClientId
              );
  ...
  const printBodyTable = (category, index) => {
    return (
      <tr key={index} role="row" className="odd">
        <td data-sort="name">
          {category && category.name && category.name.fr
            ? category.name.fr
            : ""}
        </td>
        <td data-sort="description">
          {category && category.description && category.description.fr
    ...
    verifyPermission(props.userPermissions, "list-category-client") ? (
      ...
      >
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
          <InfirmationTable information={t("Liste des catégories client")} />
                  ...
                  <div className="row">
                    <div className="col-sm-12 col-md-5">
                      ...
                      >
                        ...
                        {t("sur")} {categoryClient.length} {t("données")}
                      </div>
                    </div>
                    {showList.length ? (
