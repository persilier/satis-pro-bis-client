...
const ClaimReportingUemoaThree = (props) => {
  ...
  const fetchData = async (click = false) => {
    ...
    if (
      ...
      else endpoint = `${appConfig.apiDomaine}/without/uemoa/state-out-time`;
      sendData = {
  ...
  const downloadReportingPdf = async () => {
    ...
    if (
      ...
      if (props.plan === "MACRO")
        endpoint = `${appConfig.apiDomaine}/any/uemoa/state-out-time-pdf`;
      else
        endpoint = `${appConfig.apiDomaine}/without/uemoa/state-out-time-pdf`;
      sendData = {
        date_start: dateStart ? dateStart : null,
        date_end: dateEnd ? dateEnd : null,
        institution_id: institution ? institution.value : null,
    ...
    if (verifyTokenExpire()) {
      await axios({
        ...
        .then(async ({ data }) => {
          ...
          FileSaver.saveAs(
            data,
            `reporting_etat_hors_delai_${new Date().getFullYear()}.pdf`
          );
      ...
      <tr key={index} role="row" className="odd">
        <td>
          ...
          <div className="dropdown-menu px-5" style={{ width: "550px" }}>
            ...
            <div className="d-flex justify-content-between">
              <strong>
                {"Délai de traitement"} (J) {t("avec weekend")}
              </strong>
              <p className="ml-5">
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
          <InfirmationTable
            information={
              <div>
                {t(
                  "État complet de toutes les réclamations dont les délais de traitement sont dépassés et qui n'ont pas encore été traitées à la date du jour"
                )}
                .
              </div>
            }
          ...
          <div className="kt-portlet">
            <HeaderTablePage title={t("Rapport Etat hors délai")} />
