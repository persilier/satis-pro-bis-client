...
const Position = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      await axios
        .get(`${appConfig.apiDomaine}/positions`)
        .then((response) => {
          setNumberPage(
  ...
  const deletePosition = (positionId, index) => {
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (verifyTokenExpire()) {
        if (result.value) {
          axios
            .delete(`${appConfig.apiDomaine}/positions/${positionId}`)
            .then((response) => {
  ...
  const printBodyTable = (position, index) => {
    return (
      <tr key={index} role="row" className="odd">
        <td data-sort="name">{position.name["fr"]}</td>
        <td>
          {verifyPermission(props.userPermissions, "update-position") ? (
            <Link
              to={`/settings/positions/${position.id}/edit`}
                  ...
                  <div className="row">
                    <div className="col-sm-12 col-md-5">
                      ...
                      >
                        ...
                        {t("sur")} {positions.length} {t("données")}
                      </div>
                    </div>
                    {showList.length ? (
